...
@testset "unary - $T" for T in float_types
    @testset "$f" for f in [
        ...
        broken = ispocl && T == Float16 && f in [ceil, abs, floor, round, sqrt, trunc] && Sys.ARCH == :aarch64
...
@testset "binary - $T" for T in float_types
    @testset "$f" for f in [
        ...
        x = rand(T)
        y = rand(T)
        broken = ispocl && T == Float16 && f == atan && Sys.ARCH == :aarch64
...
@testset "OpenCL-specific unary - $T" for T in float_types
    @testset "$f" for f in [
        ...
        x = rand(T)
        broken = ispocl && T == Float16 && (f in [OpenCL.logb])
